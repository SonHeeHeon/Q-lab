...
> **Status**: ✅ **V1.1 STABILIZED** — Phase 5 complete (9 screens) + 16-bug audit cleanup (3 BLOCKING / 4 HIGH / 5 MEDIUM / 4 LOW) + SharedPreferences persistence + test seed suite. Next phases = V2 tracks per `~/.claude/plans/kis-open-api-velvety-cosmos.md` (Track-2 호가창+캔들 / Track-4 퀀트 파워 / Track-5 종목 검색).
...
- **[IN PROGRESS] Phase 5 (Flutter UI):** **[CLAUDE CODE]**
  ...
  - ✅ **Step 9 — V1.1 안정화 (2026-06-16)**: 16-bug audit cleanup. 🔴 **A1** `HeatmapNotifier` `ref.onDispose` 누적 가드 + `_disposeRegistered` flag. **A2** `AlertCondition.fromWire` / `KisAccount.fromWire` 에 `orElse` 추가 — 미지 wire 값에서 더 이상 크래시 안 함. **A3** `QuotesNotifier._onMessage` 에 `_disposed` 가드 + `_alertController.isClosed` 체크. 🟠 **A4** `showOrderSheet` 가 `whenComplete` 으로 unsubscribe (`QuotesNotifier.isSubscribed` 헬퍼 추가). **A5** `BuilderNotifier.addFactor` 가 카탈로그 소진 시 no-op + UI 버튼 disabled (`catalogExhausted`). **A6** `_HoldingRow._maybeFlash` `mounted` 가드. **A7** trade_journal 빈 `${''}` interpolation 정리. 🟡 **A8** Home 알림 fetch 가 404 와 5xx 를 분기 (`HomeSnapshot.alertsError`). **A9+A12** 신규 `lib/data/parse_utils.dart::safeDouble/safeDoubleOrNull/safeInt` — Pydantic Decimal 문자열 `"0E-8"` / `"N/A"` 등 malformed 값을 silently 0 처리하던 패턴을 통합 + `debugPrint` 가시화. `portfolio_api/backtest_api/trade_journal_api/position.dart` 모두 마이그레이션. **A10** `HeatmapLevel` enum (root/group/stock/unknown) — 백엔드 신규 level 이 silently stock 으로 새지 않음. **A11** `asJsonMap` numeric key debugAssert. 🟢 **A13** SharedPreferences 영속화: 신규 `lib/core/preferences.dart` 에 `persistedThemeModeProvider` / `persistedActiveAccountProvider`, `main.dart` 가 `SharedPreferences.getInstance()` 를 await 후 `ProviderScope.overrides` 로 주입. **A14** Heatmap toggle 300ms debounce. **A15** Principles 성공/실패 SnackBar + `context.mounted`. **A16** Watchlist FAB 가 categories + entries 둘 다 ready 일 때만 활성. **테스트 시드**: `test/parse_utils_test.dart`, `test/fromwire_fallback_test.dart`, `test/builder_factor_guard_test.dart` — V1.2 부터 widget/golden 테스트 확장 예정.
  - ✅ Step 8 (2026-06-09 — post-launch polish): **Live heatmap session UI** — `MarketSession` enum (PRE_MARKET/REGULAR/AFTER_HOURS/CLOSED) parsed from new `market_session` + `updated_at` + `source` fields. `SessionBadge` widget shows "🕒 HH:mm 기준 (정규장)" with green-pulse / blue / orange / gray color coding by session, ℹ️ tap-to-show tooltip on extended hours ("시간외 거래는 유동성 부족으로 가격 변동성이 클 수 있습니다"), compact refresh icon. **Polling pipeline** — `HeatmapNotifier` (AsyncNotifier) holds a 3-min `Timer.periodic` that calls `refresh()` without resetting state to loading → previous cells stay visible. **Smooth transitions** — Treemap gained `animate: true` + `keyOf: (n) => n.id` flags: per-cell `AnimatedPositioned` + `AnimatedContainer` (600ms easeOutCubic) so size/color glides between polls instead of flashing. Applied to both dedicated `/heatmap` and the mini-heatmap on `/quant`
  - ✅ **Step 7 (2026-06-09 — Phase 5 FINALE)**: **Equity Curve chart** — `_EquityCurveCard` w/ `fl_chart` LineChart on backtest detail (red↑/blue↓ gradient fill, dashed initial-NAV baseline, hover tooltip with date+NAV+%, 4-step date axis, smart NAV labels e.g. "1.5억"). Result trades + warnings cards bonus-rendered for fresh runs. **Sparkline widget** — pure CustomPainter w/ baseline + gradient fill + last-point dot, deterministic dummy data seeded by stock_code (`Sparkline.fromCodeDummy`). Embedded in Portfolio order BottomSheet header (placeholder pending backend price-history endpoint). **Equation Builder** at `/quant/builder` — full StrategyDefinition composer: meta (name/description), universe ChoiceChip (KOSPI200/KOSPI_ALL/KOSDAQ_ALL/CUSTOM), rebalance ChoiceChip (월/분기/연), **factor section** with curated catalog (MOMENTUM_1M/3M/6M/12M, PER/PBR/PSR, ROE/ROA, 배당) + per-factor weight slider (0–100%) + transform dropdown (RAW/ZSCORE/RANK) + 합계 라이브 표시 + [정규화] auto-normalize button, **filter section** with 6 field options + 4 ops + numeric value, **Top N** stepper + date range pickers, live JSON preview card, [백테스트 실행] FAB → `POST /api/backtest/run` w/ 5-min timeout → caches full result in `recentRunResultsProvider` (in-memory map by run_id) → auto-navigates to detail screen where equity curve renders immediately
