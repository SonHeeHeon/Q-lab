...
async def get_heatmap(
    market: Literal["KOSPI", "KOSDAQ", "NASDAQ100"] = Query(default="KOSPI"),
    group_by: Literal["sector", "industry"] = Query(default="sector"),
    as_of: Date | None = Query(default=None),
    force_refresh: bool = Query(default=False),
) -> ApiEnvelope[HeatmapResponse]:
    if market == "NASDAQ100":
        # US universe lives in research.db daily-close tables only — no KR
        # market session, Naver snapshot, or KIS intraday path applies here.
        response = await asyncio.to_thread(_build_us_heatmap, market, group_by, as_of)
...
def _build_us_heatmap(
    market: str,
    group_by: str,
    as_of: Date | None = None,
    warning: str | None = None,
) -> HeatmapResponse:
    """Build the NASDAQ100 heatmap from research.db US daily-close tables.

    Mirrors ``_build_heatmap``'s node assembly but with US-aware loaders (no
    zfill, ``stocks_us`` / ``prices_daily_us`` keyed on ``ticker``, no market
    caps). US-neutral response fields: session CLOSED, source research_db.
    """

    selected_date = as_of or _latest_us_price_date()
    served_at = _now()
    if selected_date is None:
        return HeatmapResponse(
            market=market,
    ...
    meta = _load_stock_meta_us()
    prices = _load_price_changes_us(selected_date)
    ...
    for code, stock_meta in meta.items():
        ...
        if price_row is None:
            continue
        # No US market caps — size by traded value (close x volume).
        size = float(price_row["close"] * max(price_row["volume"], 1))
        group_label = stock_meta.get(group_by) or "Unknown"
        stock_nodes.append(
            HeatmapNode(
                id=f"stock:{code}",
                ...
                meta={
                    "code": code,
                    "name": stock_meta["name"],
                    "korean_name": stock_meta.get("korean_name"),
                    "market": stock_meta["market"],
                    "sector": stock_meta.get("sector"),
                    "industry": stock_meta.get("industry"),
                    "market_cap": None,
...
def _latest_us_price_date() -> Date | None:
    with sqlite3.connect(research_db_path) as conn:
        row = conn.execute("SELECT MAX(date) FROM prices_daily_us").fetchone()
    if row is None or row[0] is None:
        return None
    return Date.fromisoformat(str(row[0]))
...
def _load_stock_meta_us() -> dict[str, dict[str, str | None]]:
    """NASDAQ100 metadata from stocks_us (exchange='NASDAQ' rows).

    Mirrors get_universe('NASDAQ100') == _us_universe(exchange='NASDAQ'). Keyed
    on raw ``ticker`` (NO zfill — US tickers are alphabetic). ``market`` carries
    the exchange so node meta stays shape-compatible with the KR loader.
    """

    sql = """
        SELECT ticker, name, exchange, sector, industry, korean_name
        FROM stocks_us
    ...
    return {
        ...
        for ticker, name, exchange, sector, industry, korean_name in rows
    }


def _load_price_changes_us(as_of: Date) -> dict[str, dict[str, float]]:
    """Latest-vs-previous close change% per US ticker from prices_daily_us.

    Same latest-2-rows logic as ``_load_price_changes`` but keyed on ``ticker``
    (NO zfill) against the US price table.
    """
    ...
    grouped: dict[str, list[tuple[str, float, float]]] = {}
    for ticker, row_date, close, volume in rows:
