...
research.db / service.db
        ...
        +--> FastAPI 백엔드
                ...
                |
                +--> KIS Open API: 잔고, 주문, 실시간 시세, 주문 동기화
                |
...
- 운영합니다: KIS 잔고/주문, 주문 체결 추적, 외부 주문 동기화
...
| KIS 연동 | 한국투자증권 Open API로 잔고, 주문, 체결내역, 실시간 시세 연결 |
...
| `TOKEN_CACHE_DIR` | `data/tokens` | KIS 토큰 캐시 |
...
목표: KIS 계좌를 연결해 잔고, 보유종목, 주문 내역 동기화만 사용합니다.

1. `.env`에 `KIS_PAPER_*` 값을 입력합니다.
2. 백엔드를 실행합니다.
3. Swagger에서 잔고를 확인합니다.
